---
modified: 2021-09-28T07:45:35+02:00
---

# Software

Useful software and frameworks

## Astronomy
- [Ekos Script](https://github.com/MattBlack85/ekos_scripts)

## Dotfiles
- [denji/vim-config-frontend](https://github.com/denji/vim-config-frontend)

## Games

- [Game of stones](http://gos.talij.com/)
- [Coding game](https://www.codingame.com/start)
- [Break The Code 2 by .Tech Domains](https://breakthecode.tech/)

## Programming

### Golang
- [Ping package](https://pkg.go.dev/github.com/sparrc/go-ping)

### Python
- [PyInstaller](http://www.pyinstaller.org)
- [Jedi installation](https://jedi.readthedocs.io/en/latest/docs/installation.html)
- [Pyupgrade](https://github.com/asottile/pyupgrade)
- [Python mini projects](https://github.com/Python-World/python-mini-projects)
- [Intermediate Python](https://t.me/link_ziriuz84/50)
- [Python Cheatsheet](https://t.me/link_ziriuz84/51)
- [Urwid](http://urwid.org/)
- [10 Tools Python Programmers Can Learn in 2022](https://dev.to/javinpaul/10-tools-python-programmers-can-learn-in-2022-49ll)
- [5 Python open-source tools to extract text and tabular data from PDF Files](https://towardsdatascience.com/5-python-open-source-tools-to-extract-text-and-tabular-data-from-pdf-files-9359cdde6b56)
- [Py2app](https://py2app.readthedocs.io/en/latest/examples.html)
- [HTTPX](https://www.python-httpx.org/)
- [SnakeViz](https://jiffyclub.github.io/snakeviz/)
- [Textualize/textual: Textual is a TUI (Text User Interface) framework](https://github.com/Textualize/textual)
- [PyScript | Run Python in your HTML](https://pyscript.net/)
- [codophobia/python-code-snippets: Code snippets on how to use different features in Python 3](https://github.com/codophobia/python-code-snippets)

### CSS
- [CSSFX](https://cssfx.netlify.app/)
- [Bootstrap Cheatsheet](https://websitesetup.org/wp-content/uploads/2020/03/Bootstrap-Cheat-Sheet-websitesetup.org_.pdf)
- [Hover](https://github.com/IanLunn/Hover)
- [LayoutIt - Bootstrap 4 Interface Builder](https://layoutit.com/build#)
- [Layoutit Grid — CSS Grids layouts made easy!](https://grid.layoutit.com/)

### JS

- [Gulp.js](https://gulpjs.com/)
- [reveal.js](https://revealjs.com/)

### Markdown

- [Marp: Markdown Presentation Ecosystem](https://marp.app/)

### Jekyll

- [Jekyll-menus](https://github.com/forestryio/jekyll-menus)

### Vim

-[Modern TypeScript and React Development in Vim](https://thoughtbot.com/blog/modern-typescript-and-react-development-in-vim)

### Modeling Languages

- [Gaphor: Open Source Graphical Modeling Tool](https://itsfoss.com/gaphor-modeling-tool/)

## Utility

### CLI

- [exa · a modern replacement for ls](https://the.exa.website/)
- [File Analyzer](https://github.com/QuattroMusic/FileAnalyzer)
- [Todux](https://github.com/sf3ris/todux)
- [Visidata](https://github.com/saulpw/visidata)
- [Bat](https://github.com/sharkdp/bat)
- [5 Rust tools worth trying on the Linux command line](https://opensource.com/article/21/7/rust-tools-linux)
- [20 one-line Linux commands to add to your toolbox](https://www.redhat.com/sysadmin/one-line-linux-commands)
- [Powerful Terminal And Command-Line (CLI) Tools For Modern Web Development](https://www.smashingmagazine.com/2021/11/powerful-terminal-commandline-tools-modern-web-development/)
- [Neofetch for Git repository](https://github.com/o2sh/onefetch)
- [nicolargo/glances: Glances an Eye on your system](https://github.com/nicolargo/glances)
- [onceupon/Bash-Oneliner: A collection of handy Bash One-Liners and terminal tricks for data processing and Linux system maintenance.](https://github.com/onceupon/Bash-Oneliner#hardware)]

## Vim plugin

- [yuki-yano/fzf-preview.vim](https://github.com/yuki-yano/fzf-preview.vim)
- [junegunn/fzf.vim](https://github.com/junegunn/fzf.vim#preview-window)
- [turbio/bracey.vim](https://github.com/turbio/bracey.vim)
- [rhysd/git-messenger.vim](https://github.com/rhysd/git-messenger.vim)
- [ap/vim-css-color](https://github.com/ap/vim-css-color)
- [mileszs/ack.vim](https://github.com/mileszs/ack.vim)
- [tpope/vim-capslock](https://github.com/tpope/vim-capslock)

## Web Services

### CMS

- [DatoCMS](http://www.datocms.com/)
- [Netlify CMS](https://www.netlifycms.org/)

### Hacktoberfest

- [Hacktoberfest Finder](https://hacktoberfest-finder.netlify.app/)

### Public Useful API

- [Weather API](https://github.com/chubin/wttr.in)
- [Comuni JSON](https://github.com/matteocontrini/comuni-json)
- [Github Readme Stats](https://github.com/anuraghazra/github-readme-stats)
- [7Timer! - numerical weather forecast for anywhere over the world](https://www.7timer.info/doc.php?lang=en#api)

### AWS

- [Localstack](https://github.com/localstack/localstack)

### Self-hosted

- [Umbrel](https://github.com/getumbrel/umbrel)
- [Radicale - Free and Open-Source CalDAV and CardDAV Server](https://radicale.org/)
- [n8n-io/n8n: Free and source-available fair-code licensed workflow automation tool.](https://github.com/n8n-io/n8n)
- [Snibox - A self-hosted snippet manager](https://snibox.github.io)

### Open source contribution

- [Codetriage](https://www.codetriage.com/)
- [TODO](https://www.linux.it/todo/)

### Google Alternatives

- [deGoogle](https://degoogle.jmoore.dev/)
- [Service categories | European Alternatives](https://european-alternatives.eu/categories)

## Various
- [Qrcode generator with api](https://qrcode.show/)
- [Semmle code analyzer](https://lgtm.com/)
- [How to Generate Temporary Email Addresses Using the Linux Terminal](https://www.makeuseof.com/generate-temporary-email-addresses-using-linux-command-line/)
- [KeepassXC Integration in Qutebrowser](https://github.com/ususdei/qute-keepassxc)
- [Stocazzo as a Service](https://github.com/dottorblaster/stocazzo)
- [README.md editor](https://github.com/octokatherine/readme.so)
- [stevemao/github-issue-templates: A collection of GitHub issue and pull request templates](https://github.com/stevemao/github-issue-templates/)
- [Catima, carte fedeltà sullo smartphone - Le Alternative](https://www.lealternative.net/2020/01/31/catima/)
